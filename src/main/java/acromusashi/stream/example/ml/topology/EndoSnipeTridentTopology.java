...
import acromusashi.stream.example.ml.trident.EndoSnipeNotifyFunction;
 ...
 * <li>EndoSnipeNotifyFunctionにて統計結果をENdoSnipe(DataCollector)に通知する。</li>
 * </ol>
 * 
 ...
 * @author kimura
 */
public class EndoSnipeTridentTopology extends BaseTridentTopology
{
    /**
     ...
     */
    public EndoSnipeTridentTopology(String topologyName, Config config)
    {
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if (args.length < 2)
        {
            System.out.println("Usage: java " + EndoSnipeTridentTopology.class.getName()
        ...
        // Topologyを起動する
        EndoSnipeTridentTopology dcTridentTopology = new EndoSnipeTridentTopology(
                "EndoSnipeTridentTopology", conf);
    ...
    public StormTopology buildTridentTopology()
    {
        ...
        // EndoSnipeNotifyFunction用設定値読み込み
        String dcHost = StormConfigUtil.getStringValue(this.config, "datacollector.host", "localhost");
        int dcPort = StormConfigUtil.getIntValue(this.config, "datacollector.port", 0);
        String messageName = StormConfigUtil.getStringValue(this.config, "datacollector.messagename", "/Trident/Performance/");
        String agentName = StormConfigUtil.getStringValue(this.config, "datacollector.agentname", "/Performance");
        // @formatter:on
        ...
        // EndoSnipeNotifyFunctionを初期化
        EndoSnipeNotifyFunction esFunction = new EndoSnipeNotifyFunction();
        esFunction.setDcHost(dcHost);
        esFunction.setDcPort(dcPort);
        esFunction.setMessageName(messageName);
        esFunction.setAgentName(agentName);
        ...
        //
        // >(each:各メッセージに対して実行)
        // 5.EndoSnipeNotifyFunction:受信した統計値をDataCollectorに通知
        // @formatter:off
        topology.newStream("TridentKafkaSpout", tridentKafkaSpout).parallelismHint(parallelism)
            ...
            .each(new Fields("average"), esFunction, new Fields("count"));
        // @formatter:on
